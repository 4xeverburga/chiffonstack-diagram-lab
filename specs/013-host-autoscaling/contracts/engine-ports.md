...
| Type | 011 | 013 |
|---|---|---|
| `HostNodeSim` (saturating variants) | manual/calculated fields | + `minReplicas: number`, `maxReplicas: number` (integers, 1 ≤ min ≤ max), `bootDelayMs: number` (≥ 0), `highWatermark: number`, `lowWatermark: number` (≥ 0, lowWatermark < highWatermark) |
...
4. **Rate limits**: at most one scaling action per host per `AUTOSCALE_COOLDOWN_MS`; actions require the host's own `highWatermark`/`lowWatermark` (user-facing capability parameters, constitution v3.3.0) condition to hold for `AUTOSCALE_SUSTAIN_MS` of simulated time; the band between them never scales (hysteresis). A single action's SIZE is proportional (`ceil(count * saturation / watermark)`, research D9), not fixed at ±1.
