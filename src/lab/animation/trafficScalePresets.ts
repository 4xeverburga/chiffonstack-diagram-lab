// Presentation-only tuning for HeatEdge's throughput->animation mapping
// (moved out of engine/config.ts per assessment.md A3: this is visual
// math, not simulation physics — the engine itself has no notion of "peak"
// traffic. CLAUDE.md: centralize any parameter on a config file, so this
// stays the single source of truth for every animation-presentation dial.
